...
# binding value, (f) the M23 suite pins, (g) the M24 suite pins, (h) the
# M25 suite pins, (i) the M26 chain pins, (j) the M27 tcb pins and (k)
# the M28 attest pins. A
...
# command for it.
# tcbx.ml is the FIFTH attestation-tower module (M27): quotex DECODES
# the bytes, policyx DECIDES on the body, sigx PROVES the signature
# section, derx CHAINS the platform key to the pinned Intel root and
# this unit GRADES the platform TCB and the quoting enclave against
# Intel's signed collateral, pure and sans-io in the same shape, no
# Bytes, no Buffer, no Array, no reference cell, no division line and
# no remainder, and no loop keyword. It reads the collateral
# envelope, the TCB Info and QE Identity documents inside it with
# their two signing chains, and the pck key, the qe report and the
# body of the quote, through jsonx, hexx, derx, sigx, quotex, p256x,
# bytesx and errx, and trap 2 fires CROSS-MODULE, so this file is
# linted beside all five and never alone. ONE layout record built by
# layout () carries every report offset, every member name, every
# status word and every pinned string, and it is passed FIRST to
# every helper, so no helper reads a top-level constant under trap 2
# and no numeric literal sits outside that record. It calls
# P256x.verify_message at TWO call sites that run FOUR times, one
# signing certificate tbs leg under the pinned root key and one
# document leg under the signing key, for each of the two issuer
# chains in the envelope, and it calls Sha2 NEVER and B64x NEVER,
# because the p256x message verify hashes every message itself and
# the ONE block this unit parses goes through Derx.Cert.of_pem, which
# owns the base64. The unit applies no functor and holds no Map. The
