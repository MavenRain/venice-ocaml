...
| M24 | policy: DEBUG reject, nonce + signing_key REPORTDATA binding, Expect.make ~measurements (tofu mints structural only) + tests;  the W1 binding is `report_data = address20 || 0x00 x 12 || nonce32`, the nonce RAW at REPORTDATA[32..64], the address at [0..20] and bytes 20..32 zero on the ECDSA path;  the RAW compare stands alone, because the consumer also retries `sha256(nonce)` and the producer never builds that |
...
copied constant.  A probe milestone lands
no OCaml unit, so it has no behavioral mutant:  M22 satisfies
MUTATION-CONFIRMED with the teeth of its brief, each of which makes the
gate RED on a scratch copy and GREEN again on the untouched tree.
