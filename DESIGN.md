...
| Psychic signatures (r = 0 or s = 0) | CVE-2022-21449 (Java), where a zero pair verified under every key | p256x mints a `Signature.t` only when r and s both sit in 1 .. n-1, so r = 0, s = 0, r >= n and s >= n are rejected at CONSTRUCTION, before any point arithmetic runs;  the verifier is variable-time by design, because it reads only PUBLIC inputs (attestation quotes, certificates, signatures) |
