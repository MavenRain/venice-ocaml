...
| M23 | quotex: TDX quote header + TD report body (mrtd, rtmr0-3, report_data, td_attributes, xfam, tee_tcb_svn) via bytesx cursors + fixture tests;  version 4 only, att_key_type 2 only, tee_type 0x00000081 only, 636 + signature_data_len <= length with the surplus recorded and never consumed, cert_key_type 6 with inner type 5 only and every other type a typed reject, the PEM chain split on the certificate markers, version 5 rejected by version with a typed reason, fixtures tdx_quote_v4.bin and tdx_quote_v5.bin |
...
REPORTDATA formula before anything is pinned.  A probe milestone lands
